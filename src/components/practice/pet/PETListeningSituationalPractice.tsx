...
export interface PETListeningSituationalPracticeProps {
  onBack: () => void;
  sessionId?: string;
...
interface RestoredState {
  ...
  framingText: string;
  itemResults: PETSituationalItemResult[] | null;
  correctCount: number;
}
...
function tryRestore(messages: StoredMessage[]): RestoredState | null {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === 'bob' && msg.msg_type === 'evaluation' && cj.is_final === true) {
      itemResults = cj.item_results as PETSituationalItemResult[];
...
function TranscriptBlock({ turns }: { turns: PETSituationalClientItem['conversation'] }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 overflow-hidden bg-white">
      ...
      {open && (
        <div className="px-4 pb-3 space-y-1.5 border-t border-gray-100 pt-2.5">
          {turns.map((turn, i) => (
...
function ResultQuestionCard({
  ...
  item: PETSituationalClientItem;
  result: PETSituationalItemResult;
  index: number;
  animate: boolean;
  reduceMotion: boolean;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3">
        <TranscriptBlock turns={item.conversation} />
      </div>
    </motion.div>
  );
}
...
export function PETListeningSituationalPractice({
  ...
  onSessionFinished,
  onOpenDashboard,
}: PETListeningSituationalPracticeProps) {
  const reduceMotion = useReducedMotion();
