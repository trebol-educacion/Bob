...
import {
  ...
  generatePETListeningAttitudeAudioAction,
...
function tryRestore(messages: StoredMessage[]): RestoredState | null {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === 'bob' && cj.kind === 'pet_listening_attitude_plan') {
      const raw = (cj.items as Array<Omit<PETAttitudeClientItem, 'audio_b64' | 'audio_mime'>>) ?? [];
      items = raw.map((item) => ({
        ...item,
        audio_b64: '',
...
function AudioPlayer({
  ...
  const label = playing ? 'Playing…' : hasPlayed ? 'Listen again' : 'Listen to the speaker';
...
export function PETListeningAttitudePractice({
  ...
  useEffect(() => {
    ...
    for (const item of items) {
      (async () => {
        try {
          const audio = await generatePETListeningAttitudeAudioAction({ monologue: item.monologue });
          if (cancelled) return;
          setAudioByItem((prev) => ({
            ...prev,
