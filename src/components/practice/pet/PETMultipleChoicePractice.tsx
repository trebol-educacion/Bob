...
export interface PETMultipleChoicePracticeProps {
  onBack: () => void;
  sessionId?: string;
...
function tryRestore(messages: StoredMessage[]): RestoredState | null {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === 'bob' && cj.kind === 'pet_listening_part2_plan') {
    ...
    if (
      ...
      cj.kind === 'pet_listening_turn_result' &&
      cj.is_final === false
    ) {
      turns.push({
        item_id: String(cj.item_id),
        selected_key: String(cj.selected_key),
        correct_key: String(cj.correct_key),
        ...
        return (
          <div
            ...
            className={[
              ...
              isWrong ? 'bg-red-400' : '',
              isCurrent ? 'bg-bob-brand ring-2 ring-offset-1 ring-bob-brand/40' : '',
...
export function PETMultipleChoicePractice({
  ...
  onSessionFinished,
  onOpenDashboard,
}: PETMultipleChoicePracticeProps) {
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col items-center justify-center flex-1 p-8 gap-6 text-center">
          ...
          <button
            type="button"
            onClick={handleRetry}
            className="flex items-center gap-2 px-5 py-2.5 bg-bob-brand text-white rounded-xl font-semibold text-sm hover:opacity-90 transition cursor-pointer"
          >
            <RotateCcw size={16} />
            Try again
          </button>
            ...
            return (
              ...
              >
                <div className="px-4 pt-4 pb-2 flex items-center gap-2">
                  <span
                    className="w-6 h-6 rounded-full text-xs font-black flex items-center justify-center shrink-0 text-bob-brand"
                ...
                <div className="px-4 pb-4 space-y-1.5 mt-2">
                  {item.options.map((opt) => {
                    const isChosen = opt.key === turn.selected_key;
                    const isCorrectOpt = opt.key === turn.correct_key;
                    return (
                      <div
                        key={opt.key}
                        className={[
                      ...
                      {currentItem.options.map((opt) => (
                        <button
                          ...
                          className={[
                            ...
                            selectedKey === opt.key
                              ? 'border-bob-brand bg-bob-brand/5 text-bob-brand'
          ...
          <div className="shrink-0 border-t border-gray-100 bg-white px-4 py-3 flex items-center gap-3 max-w-3xl mx-auto w-full">
            <button
              ...
              className={[
                ...
                selectedKey
                  ? 'bg-bob-brand text-white hover:opacity-90 shadow-sm'
                  : 'bg-gray-100 text-gray-400 cursor-not-allowed',
              ].join(' ')}
            >
