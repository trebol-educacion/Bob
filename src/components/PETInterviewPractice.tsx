...
export function PETInterviewPractice({ onBack }: PETInterviewPracticeProps) {
  ...
  const [plan, setPlan] = useState<PETInterviewPlan | null>(null);
  ...
  const startSession = useCallback(async () => {
    try {
      ...
      const sessionPlan = await generatePETInterviewAction(sessionIdRef.current, userIdRef.current);
      setPlan(sessionPlan);
      setQuestions(petInterviewQuestions(sessionPlan));
      setStep('answer');
      setReady(true);
    } catch (err) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (blob && blob.size > 0) {
        ...
        const result = await processPETInterviewAnswerAction(
          audioBase64,
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const result = await evaluatePETInterviewAction(qas, sessionIdRef.current, userIdRef.current);
  ...
  const header = (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-white shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-gray-800 truncate">Speaking · Part 1: Interview</p>
  ...
  if (!ready) {
    return <BobMascotLoader message="Getting your interview ready…" />;
  }
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          ...
          >
            <div className="text-center space-y-1">
              ...
              <p className="text-sm text-gray-400 font-medium">Cambridge B1 · Part 1</p>
            </div>
